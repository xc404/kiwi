...
- [x] 2.2 在 `ExternalTaskRetryProperties` 新增 `nonDecreasingRetryCycle`（默认 `PT30S`，纯 ISO duration）；不引入 FQCN 字符串匹配
...
- [x] 4.3 退避周期解析失败 / 配置缺失时回退到 BPMN / 引擎默认 cycle 的第一个间隔；二者都失败时兜底 30000ms；首次失败 `nextRetries` 固定为 `1`（与 `RetryPlannerTest` 一致）
...
- [x] 5.4 手工冒烟（可选）：设置 `kiwi.bpm.slurm.max-concurrent-jobs=1`，启动两个并发流程，验证第二个任务出现 `WARN Slurm overloaded` 且 ~30s 后被重试拉起
