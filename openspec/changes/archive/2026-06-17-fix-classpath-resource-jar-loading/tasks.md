...
- [x] 2.2 运行 `mvn -q compile -DskipTests`（`kiwi-admin/backend` 或根模块按项目惯例）确认编译通过。

## 3. 验证

- [x] 3.1 （可选）本地打包 JAR 后启动或最小化集成验证，确认 `classpath:bpm/bpm-template.xml` 与权限 JSON 加载无 `FileNotFoundException`。
