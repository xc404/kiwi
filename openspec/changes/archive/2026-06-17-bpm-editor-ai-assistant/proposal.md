...
## Related changes（与本 BPM AI 功能无直接耦合）

CryoEMS ↔ Kiwi 集成在配置位置（`Task` 级流程 id）、`MovieKiwiWorkflowService` 入参（`task` / `taskDataset`）、以及 **`KiwiWorkflowClient` 遇 429 重试** 等方面的契约修正，见变更目录 **`openspec/changes/cyroems-kiwi-workflow-integration`** 中的 `proposal.md`。
