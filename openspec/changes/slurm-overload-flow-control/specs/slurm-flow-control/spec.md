...
系统 SHALL 在 `ExternalTaskRetryPlanner.plan(task, failure)` 中，沿失败异常链定位首个 `IRetry` 实例；当其 `decreaseRetries()` 返回 `false` 时 MUST 保留 External Task 当前的 `retries` 值；当 `retries` 为 `null`（首次失败）时 MUST 使用 `kiwi.bpm.external-task-retry.non-decreasing-retry-cycle`（默认 `R5/PT30S`）的 `retries` 字段作为初值。
...
#### Scenario: 首次失败使用 cycle retries 初值

- **WHEN** `task.getRetries()` 为 `null` 且本次失败为 `SlurmOverloadedException`
- **THEN** 返回的 `nextRetries` 等于 `non-decreasing-retry-cycle` 解析出的 `retries` 字段（默认 5）
...
系统 SHALL 在非递减重试分支优先使用 `kiwi.bpm.external-task-retry.non-decreasing-retry-cycle` 计算 `retryTimeoutMs`；当其为空或解析失败时 MUST 回退到 BPMN / 引擎默认 cycle 的第一个间隔；当二者都解析失败时 MUST 兜底为 30000 毫秒。

#### Scenario: 解析配置的退避周期

- **WHEN** `kiwi.bpm.external-task-retry.non-decreasing-retry-cycle = R5/PT30S` 且本次失败为 `SlurmOverloadedException`
