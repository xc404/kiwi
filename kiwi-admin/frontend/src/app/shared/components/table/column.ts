...
@Component({
  ...
  styles: '',
  template: ` <app-button [config]="action()"></app-button> `,
...
@Component({
  ...
  template: `
    @for (action of actions(); track $index) {
      <app-table-column-action class="m-r-10" [action]="action" [record]="record()"></app-table-column-action>
    }
  `,
...
@Component({
  ...
  styles: '',
  template: ` @if (!this.field().thTemplate) {
      <span>
        {{ name() }}
      </span>
    } @else {
      @if (isTemplate()) {
        <ng-container [ngTemplateOutlet]="this.field().thTemplate" [ngTemplateOutletContext]="{ value: value(), field: field(), record: record() }"></ng-container>
      } @else {
        <ng-container #instance="ngComponentOutlet" [ngComponentOutlet]="this.field().thTemplate" [ngComponentOutletInputs]="{ value: value(), field: field(), record: record() }"></ng-container>
      }
    }`,
...
@Component({
  ...
  styles: '',
  template: ` @if (!this.field().tdTemplate) {
      @if (field().dictKey) {
      ...
      @if (isTemplate()) {
        <ng-container [ngTemplateOutlet]="this.field().tdTemplate" [ngTemplateOutletContext]="{ value: value(), field: field(), record: record() }"></ng-container>
      } @else {
        <ng-container #instance="ngComponentOutlet" [ngComponentOutlet]="this.field().tdTemplate" [ngComponentOutletInputs]="{ value: value(), field: field(), record: record() }"></ng-container>
      }
    }`,
