import { CdkDrag, CdkDragDrop, CdkDragHandle, CdkDropList, moveItemInArray } from '@angular/cdk/drag-drop';
import { NgStyle } from '@angular/common';
import { AfterContentInit, booleanAttribute, ChangeDetectionStrategy, Component, computed, contentChild, input, output, TemplateRef } from '@angular/core';

import { ScreenLessHiddenDirective } from '@shared/directives/screen-less-hidden.directive';

import { NzButtonModule } from 'ng-zorro-antd/button';
import { NzCardModule } from 'ng-zorro-antd/card';
import { NzCheckboxModule } from 'ng-zorro-antd/checkbox';
import { NzSafeAny } from 'ng-zorro-antd/core/types';
import { NzDividerModule } from 'ng-zorro-antd/divider';
import { NzDropDownModule } from 'ng-zorro-antd/dropdown';
import { NzIconModule } from 'ng-zorro-antd/icon';
import { NzMenuModule } from 'ng-zorro-antd/menu';
import { NzPopoverModule } from 'ng-zorro-antd/popover';
import { NzSpaceModule } from 'ng-zorro-antd/space';
import { NzTableSize } from 'ng-zorro-antd/table';
import { ColumnConfig } from '../column';
import { TableComponentToken } from '../table';
import { AppButtonConfig, AppButton } from '../../button/app.button';
import { NzTooltipModule } from 'ng-zorro-antd/tooltip';

interface TableSizeItem {
  sizeName: string;
  selected: boolean;
  value: NzTableSize;
}

@Component({
  selector: 'app-table-wrap',
  templateUrl: './app-table-wrap.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [
    NzCardModule,
    NzDividerModule,
    NzSpaceModule,
    NzIconModule,
    NzButtonModule,
    NzPopoverModule,
    NzTooltipModule,
    NzDropDownModule,
    NzMenuModule,
    CdkDropList,
    CdkDrag,
    CdkDragHandle,
    NzCheckboxModule,
    NgStyle,
    ScreenLessHiddenDirective,
    AppButton
  ]
})
export class AppTableWrapComponent implements AfterContentInit {
  readonly tableTitle = input<string | TemplateRef<NzSafeAny>>();
  toolbarActions = input<AppButtonConfig[]>();
  readonly isNormalTable = input(true, { transform: booleanAttribute }); // 如果只是需要card-table-wrap的样式，这里设置为false
  readonly reload = output();
  readonly tableComponent = contentChild(TableComponentToken);
  tableConfigVisible = false;
  tableSizeOptions: TableSizeItem[] = [
    { sizeName: '默认', selected: true, value: 'default' },
    { sizeName: '中等', selected: false, value: 'middle' },
    { sizeName: '紧凑', selected: false, value: 'small' }
  ];
  columns: ColumnConfig[] = [];

  // 当前包裹的table组件
  currentTableComponent = computed(() => {
    const tableComponent = this.tableComponent();
    if (!tableComponent) {
      throw new Error('没有antTableComponent或者antTreeTableComponent作为投影组件');
    }
    return tableComponent;
  });

  allTableFieldChecked = false; // 设置里面全选列
  allTableFieldIndeterminate = false; // 设置里面全选列的半选状态
  initColumns: ColumnConfig[] = []; // 缓存默认配置

  // 是否展示复选框
  changeTableCheckBoxShow(e: boolean): void {
    this.currentTableComponent().tableConfig().showCheckbox = e;
    this.tableChangeDectction();
  }

  // 大中小表格密度
  tableSizeMenuClick(item: TableSizeItem): void {
    this.tableSizeOptions.forEach(tableSizeItem => (tableSizeItem.selected = false));
    item.selected = true;
    this.currentTableComponent().tableSize = item.value;
  }

  // 配置中tableCheckbox是否全选
  changeAllTableTableConfigShow(e: boolean): void {
    if (e) {
      this.allTableFieldChecked = e;
      this.allTableFieldIndeterminate = false;
    }
    this.columns.forEach(item => (item.show = e));
    this.tableChangeDectction();
  }

  // 设置固定左侧还是右侧
  changeTableConfigShow(): void {
    const tempArray = [...this.columns];
    const fixedLeftArray: ColumnConfig[] = [];
    const fixedRightArray: ColumnConfig[] = [];
    const noFixedArray: ColumnConfig[] = [];
    tempArray.forEach(item => {
      if (item.fixed) {
        if (item.fixedDir === 'left') {
          fixedLeftArray.push(item);
        } else {
          fixedRightArray.push(item);
        }
      } else {
        noFixedArray.push(item);
      }
    });
    this.currentTableComponent().tableConfig().columns = [...fixedLeftArray, ...noFixedArray, ...fixedRightArray];
    this.tableChangeDectction();
  }

  dropTableConfig(event: CdkDragDrop<string[]>): void {
    moveItemInArray(this.columns, event.previousIndex, event.currentIndex);
    this.changeTableConfigShow();
  }

  fixedTableHead(dir: 'right' | 'left', item: ColumnConfig): void {
    item.fixed = !(item.fixed && item.fixedDir === dir);
    item.fixedDir = dir;
    this.changeTableConfigShow();
  }

  reloadClick(): void {
    this.reload.emit();
  }

  // 某一列check变化
  changeSignalCheck(e: boolean, item: ColumnConfig): void {
    item.show = e;
    this.judgeAllChecked();
    this.tableChangeDectction();
  }

  // 使子列表变更检测
  tableChangeDectction(): void {
    this.currentTableComponent().tableChangeDectction();
  }

  // 判断列展示这个checkbox的状态
  judgeAllChecked(): void {
    this.allTableFieldChecked = this.columns.every(item => item.show === true);
    const allUnChecked = this.columns.every(item => !item.show);
    this.allTableFieldIndeterminate = !this.allTableFieldChecked && !allUnChecked;
  }

  // 重置
  reset(): void {
    this.columns = [];
    this.initColumns.forEach(item => {
      this.columns.push({ ...item });
    });
    this.currentTableComponent().tableConfig().columns = [...this.columns];
    this.tableChangeDectction();
  }

  ngAfterContentInit(): void {
    if (this.isNormalTable()) {
      this.columns = [...this.currentTableComponent().tableConfig().columns];
      this.columns.forEach(item => {
        if (item.show === undefined) {
          item.show = true;
        }
      });
      this.initColumns.length = 0;
      this.columns.forEach(item => {
        this.initColumns.push({ ...item });
      });
      this.judgeAllChecked();
    }
  }
}
