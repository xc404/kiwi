...
export function toFormlyConfig(config: FieldEditorConfig, wrapper = 'form-field', props?: any): FormlyFieldConfig {
  const hidden = config.hidden || props?.hidden;
  ...
  return {
    ...
    props: {
      ...
      hidden: hidden,
      ...(config.options?.length ? { options: config.options, valueProp: 'value', labelProp: 'label' } : {})
    }
  };
}
...
export function toFormlyType(editor: Editor | any): string | Type<any> {
  switch (editor) {
    ...
    case Editor.DateTime:
      return 'datepicker';
    case Editor.CheckboxGroup:
      return 'checkbox';
    case Editor.Radio:
      return 'radio';
    case Editor.CheckBox:
      return 'checkbox';
    case Editor.ComboBox:
      return 'select';
    default:
