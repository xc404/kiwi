...
export type CrudFieldConfig = ColumnConfig &
  FieldEditorConfig & {
...
export function toFormField(field: CrudFieldConfig, mode: 'search' | 'create' | 'update'): FormlyFieldConfig | null {
  if (mode === 'search') {
    ...
    const editor = (field.search === true ? undefined : field.search?.editor) || field.editor;
  ...
  let readonly = false,
    disabled = false,
    hidden = false;
  const prop = mode == 'create' ? edit.create : edit.update;
  if (prop === false) {
    return null;
  }
  const editor = edit.editor || field.editor;
  if (prop === false) {
    return null;
  }
  if (prop === 'readonly') {
  ...
  const props = { readonly, disabled, hidden };
