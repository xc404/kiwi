...
export const PaletteProviders = new InjectionToken<PaletteProvider[]>('PaletteProviders');
    ...
    .map(tab => ({
      ...
      paletteGroup: tab.paletteGroup
        .map(g => {
          ...
          const palettes = groupMatch ? g.palettes : g.palettes.filter(p => (p.title ?? '').toLowerCase().includes(needle));
          return { ...g, palettes };
        })
...
@Component({
  ...
  imports: [NzDropdownModule, NzIconModule, NzLayoutModule, NzTabsModule, NzCollapseModule, NzButtonModule, NzFormModule, DndModule, NzTooltipModule, NzInputModule, FormsModule],
  standalone: true
})
export class BpmPallete implements OnInit {
  ...
  filteredPalleteTabs = computed(() => filterPaletteTabs(this.palleteTabs(), this.searchQuery()));
  ...
  createElement(item: PaletteItem, providerIndex: number, event: any) {
    ...
    const bpmnFactory: BpmnFactory = this.bpmnModeler.get('bpmnFactory');
    const create: Create = this.bpmnModeler.get('create');
    const elementFactory: ElementFactory = this.bpmnModeler.get('elementFactory');
    const palleteProvider = this._paletteProviders[providerIndex];
    const { type, options, eventDefinitionType } = palleteProvider.getElementOptions(item);
  ...
  ngOnInit(): void {
    ...
    const ps: Array<Observable<{ name: string; paletteGroup: PaletteGroup[] }>> = this._paletteProviders.map(provider => {
      const paletteGroup = provider.getPaletteGroup();
      if (paletteGroup instanceof Observable) {
        return paletteGroup.pipe(
          map(group => {
    ...
    combineLatest(ps).subscribe(tabs => {
      this.palleteTabs.set(tabs.map((tab, providerIndex) => ({ ...tab, providerIndex })));
    });
  }
}
