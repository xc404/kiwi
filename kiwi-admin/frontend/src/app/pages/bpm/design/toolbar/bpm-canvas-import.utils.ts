...
export interface BpmCanvasImportOptions {
  /** 默认 true；AI 导入后链式保存时设为 false，避免提示「尚未保存」 */
  notifySuccess?: boolean;
}

export function importBpmnXmlToModeler(modeler: BpmnModeler, xml: string, message: NzMessageService, clearSelection: () => void, options?: BpmCanvasImportOptions): Promise<void> {
  ...
  return modeler
    ...
    .then(({ warnings }) => {
      ...
      if (warnings?.length) {
        ...
        message.warning(`已导入，存在 ${warnings.length} 条解析提示（详见控制台），请检查后保存`);
