...
@Component({
  ...
  standalone: true,
  imports: [NzDividerModule, NzButtonModule, NzIconModule, CustomOutputRowComponent, ReadonlyPropertyRowComponent],
...
export class CustomInputsPanel {
  ...
  private readonly catalogFingerprint = computed(() => [...this.catalogInputKeys()].sort().join('\0'));
  ...
  private configuredInputParameterValue(name: string): string {
    const value = this.elementModel.getValue(this.bpmnModeler(), this.element(), PropertyNamespace.inputParameter, name);
  ...
  private flush(): void {
    ...
    const existingNames = this.elementModel
      .getInputParameters(element)
      .map((p: any) => String(p.get('name')))
      .filter(name => !catalog.has(name));
    ...
    for (const [name, valueText] of desired.entries()) {
      this.elementModel.setValue(modeler, element, PropertyNamespace.inputParameter, name, valueText);
    }
  }
}
