...
import { BpmProcessInstanceDto, CamundaHistoricVariableInstance } from '../service/process-instance.service';
...
export class BpmPropertiesPanel implements OnInit {
    ...
    this.processInstanceService.getProcessInstanceVariables(id).subscribe({
      next: variables => {
        const sorted = [...variables].sort((a, b) => {
          const dt = variableCreateTimeMs(a) - variableCreateTimeMs(b);
          if (dt !== 0) {
            return dt;
          }
          const na = a.name ?? '';
  ...
  activityVariables = computed(() => {
    ...
    const processOnly = Array.from(processByName.values()).filter(v => !activityNames.has(v.name as string));
