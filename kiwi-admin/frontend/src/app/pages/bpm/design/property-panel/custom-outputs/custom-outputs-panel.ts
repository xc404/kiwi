...
export class CustomOutputsPanel {
  ...
  private readonly catalogFingerprint = computed(() => [...this.catalogOutputKeys()].sort().join('\0'));
  ...
  private configuredOutputParameterValue(name: string): string {
    const value = this.elementModel.getValue(this.bpmnModeler(), this.element(), PropertyNamespace.outputParameter, name);
