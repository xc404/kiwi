...
export abstract class ElementModel {
    ...
    private setEventValue(bpmnModeler: BaseViewer, element: Element, key: string, value: any): boolean {
        ...
        if (key === 'messageName') {
            ...
            const ref = trimmed
                ? this.findOrCreateRootElementByName(bpmnModeler, element, 'bpmn:Message', trimmed)
                : null;
        ...
        if (key === 'signalName') {
            const trimmed = value == null ? '' : String(value).trim();
            const ref = trimmed
                ? this.findOrCreateRootElementByName(bpmnModeler, element, 'bpmn:Signal', trimmed)
                : null;
            const def: any = (bo.eventDefinitions || [])[0];
            if (def) {
     ...
     */
    private findOrCreateRootElementByName(
