...
export class ProcessDesignService {
  ...
  listVisibleProcesses(query?: { projectId?: string; page?: number; size?: number }): Observable<BpmProcess[]> {
    ...
    return this.http.get<{ content?: BpmProcess[] }>(this.apiUrl, params).pipe(map(res => res?.content ?? []));
  }
   ...
   */
  saveAsComponent(processId: string, body: { name: string; description?: string; version?: string }) {
  ...
  getRecentComponentUsages(): Observable<ComponentDescription[]> {
    return this.http.get<{ content: ComponentDescription[] }>('/bpm/component/recent-usage').pipe(map(res => res.content));
  }
}
