...
export class ComponentPropertyProvider implements PropertyProvider {
  ...
  getProperties(element: Element): PropertyTab[] {
    ...
    const inputSplit = component
      ...
      : { importantGroups: [] as Array<{ name: string; properties: PropertyDescription[]; important?: boolean }>, unimportant: [] as PropertyDescription[] };

    const inputTabGroups: Array<{ name: string; properties: PropertyDescription[]; important?: boolean }> = [...inputSplit.importantGroups];
    ...
    const outputTabGroups: Array<{ name: string; properties: PropertyDescription[]; important?: boolean }> = [...outputSplit.importantGroups];
    if (outputSplit.unimportant.length > 0) {
      outputTabGroups.push({ name: '其他', properties: outputSplit.unimportant, important: false });
    }

    const tabs: PropertyTab[] = [{ name: '基础信息', groups: bindingGroups }];
    const showInputTab = inputTabGroups.length > 0 || element.type === 'bpmn:CallActivity';
    if (showInputTab) {
  ...
  private buildBindingGroups(element: Element): Array<{ name: string; properties: PropertyDescription[]; important?: boolean }> {
  ...
  private splitAndGroupByGroup(
    ...
    importantGroups: Array<{ name: string; properties: PropertyDescription[]; important?: boolean }>;
  ...
  private normalizeRequiredInputDefaults(parameters: PropertyDescription[]): PropertyDescription[] {
    return parameters.map(property => {
      ...
      const hasDefault = (typeof defaultValue === 'string' && defaultValue.trim().length > 0) || (defaultValue !== undefined && defaultValue !== null && typeof defaultValue !== 'string');
      if (hasDefault) {
        return property;
      }
      return {
