...
export class ComponentService {
  ...
  public initElement(bpmnModeler: BpmnModeler, element: Element, item: ComponentDescription | any) {
    ...
    const inputNamespace = PropertyNamespace.inputParameter;

    this.setComponentId(bpmnModeler, element, item);
    item.inputParameters?.forEach((p: PropertyDescription) => {
      const defaultValue = p.defaultValue !== undefined && p.defaultValue !== null && p.defaultValue !== '' ? p.defaultValue : `\${${p.key}}`;
      this.elementModel.setValue(bpmnModeler, element, p.namespace || inputNamespace, p.key, defaultValue);
    });
  }
