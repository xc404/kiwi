  ...
  template: `
    @if (step() === 1) {
      @if (!jdbcConnectionsChecked()) {
        ...
        <a class="m-t-16" nz-button nzType="primary" routerLink="/tools/connection" (click)="destroyModal()">前往创建数据库连接</a>
      } @else {
        <div style="width: 100%; text-align: center; ">
          <nz-form-label class="m-r-20" nz-form-label>数据库链接</nz-form-label>

          <app-dict-selector name="" storeId="jdbc-connections" [model]="connection()" (modelChange)="connection.set($event)"></app-dict-selector>
        </div>
      }
    }
