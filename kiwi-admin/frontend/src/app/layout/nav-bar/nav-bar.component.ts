...
@Component({
  ...
  imports: [NzMenuModule, NzNoAnimationDirective, NgTemplateOutlet, NzButtonModule, NzIconModule, RouterLink, AsyncPipe, AuthDirective]
})
export class NavBarComponent implements OnInit {
  ...
  cloneMenuArray(sourceMenuArray: Menu[], target: Menu[] = []): Menu[] {
    sourceMenuArray.forEach(item => {
      const obj: Menu = { name: '', menuType: 'C', path: '', id: -1, parentId: -1, code: '' };
      for (const i in item) {
