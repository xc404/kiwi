...
本目录存放 **可选** BPM 组件的 plugin JAR（Kafka、RabbitMQ、S3、Slack、Payment 等），由 `BpmComponentPluginLoader` 在启动时加载（配置项 `bpm.component.plugins-dir`，默认 `plugins`，相对**工作目录**）。

**官方核心组件**（HTTP、Shell、JDBC、Mongo 等）已改为 Maven 依赖 `kiwi-bpmn-component`，随 backend classpath 加载（`classpath_*`），**不在此目录**。
...
clone 仓库后，在 IDE 中从 **`kiwi-admin/backend`** 运行 `Application`（profile `local,dev`）即可。

- 改 **`kiwi-bpmn-component`**：重新编译 backend（`mvn -pl kiwi-admin/backend -am compile`）后重启，**无需** `build-plugins`。
- 改 **下方插件模块**：需 `mvn -Pbuild-plugins` 刷新 JAR。
...
若改动了 `kiwi-bpmn/kiwi-bpmn-component-kafka|rabbitmq|s3|slack|payment|example` 等插件模块，在仓库根目录重新打包并提交 JAR：
...
| `kiwi-bpmn-component-example-*-plugin.jar` | 第三方组件开发示例 |
