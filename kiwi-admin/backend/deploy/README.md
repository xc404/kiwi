...
产品说明与仓库结构见仓库根 [README.zh-CN.md](../../../README.zh-CN.md)（[English](../../../README.md)）。
...
构建与仓库根 [README.zh-CN.md](../../../README.zh-CN.md)（[English](../../../README.md)）一致：在 **kiwi 仓库根**执行 `mvn -pl kiwi-admin/backend -am package -DskipTests`，以按顺序编译 **kiwi-common**、**kiwi-bpmn-*** 等依赖模块。Maven 输出在 `target/`，`deploy.py` 会将其同步到 **`backend/bin/`**（应用 jar + 依赖 lib jar）再上传。请使用完整克隆的仓库，勿只拷贝 `kiwi-admin` 子目录。
