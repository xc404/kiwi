...
class AiAuthoringInstallDelegateTest {
    ...
    void execute_confirmedInstallsPluginFromCatalogMetadata() throws Exception {
        ...
        when(execution.getVariable(AiAuthoringVariables.CatalogJson)).thenReturn("""
                {
                  "installable":[{
        ...
        AiAuthoringInstallDelegate delegate =
                new AiAuthoringInstallDelegate(installService, new ObjectMapper());

        delegate.execute(execution);

        verify(installService).installPlugin("slack-plugin", "1.2.0", "official", "user-1");
        verify(execution).setVariable(AiAuthoringVariables.Stage, AiAuthoringVariables.StageInstall);
        ArgumentCaptor<Object> catalogCaptor = ArgumentCaptor.forClass(Object.class);
        verify(execution).setVariable(eq(AiAuthoringVariables.CatalogJson), catalogCaptor.capture());
        ...
        verify(execution).setVariable(AiAuthoringVariables.PluginHintJson, null);
    }
}
